...
The modular synthesizer's defining architectural choice, that every module is independently patched, creates a problem that acoustic instruments and most traditional electronic instruments do not have: there is no shared timing reference. Each module runs on its own until something tells it when. Roland addressed this in 1980 with DIN sync, introduced on the TR-808 drum machine: a two-wire protocol transmitting 24 clock pulses per quarter note and a separate run/stop signal. DIN sync allowed Roland's TR-series machines and CSQ sequencers to lock together reliably across a studio, and it held that position until MIDI's broader adoption in the mid-1980s displaced it. Eurorack, which operates on control voltage rather than digital protocols, inherited the underlying need: a single authoritative timing source that every module in a patch can track.

Matthew Allum founded ALM/Busy Circuits in the United Kingdom after building a Doepfer Eurorack system around 2010, following disappointment with the Teenage Engineering OP-1. The specific frustration that produced the first ALM module was practical: he needed to synchronize his Eurorack system with an MFB sequencer and older DIN sync equipment, and no available Eurorack module handled that cleanly. The module he built to solve that problem became the ALM001, Pamela's Workout: a compact programmable clock source with eight independent outputs, each assignable to a division or multiplication of a master BPM. It was the module that established ALM as a company. Pamela's Pro Workout is the third generation of that same original idea.

The Euclidean rhythm algorithm entered music theory formally through a 2005 paper by Godfried Toussaint, a computer scientist and ethnomusicologist at McGill University: "The Euclidean Algorithm Generates Traditional Musical Rhythms." The algorithm itself is ancient; it appears in Euclid's Elements around 300 BCE as a method for computing the greatest common divisor of two integers. Toussaint demonstrated that applying it to the problem of distributing a given number of beats as evenly as possible across a given number of steps produces rhythmic patterns that appear across world music: West African percussion, Cuban clave, Persian classical music, Turkish aksak, Balkan folk rhythm. The clave son, the bossa nova clave, and the bhangra all fall out of the same 2,300-year-old procedure. Eurorack modules incorporated Euclidean sequencing quickly after Toussaint's paper circulated in the synthesis community. Pamela's integration of Euclidean patterns into its per-output parameter set meant that each of the eight channels could generate rhythmically sophisticated gate sequences rather than simple clock divisions, and those sequences could be different on every output simultaneously.

The transition from the original Pamela's Workout to the Pro Workout traces a broader shift in how the Eurorack community understood what a clock module was for. The original solved a synchronization problem. Successive versions discovered that once you can generate any rhythmically synchronized signal at any channel, whether a gate, an LFO waveform, a probability-weighted trigger, an envelope shape, or an audio-rate tone, the boundary between clocking and modulation is a matter of rate rather than kind. The Pro Workout pushes that logic to its conclusion: audio-rate multiplication, FLEX micro-timing for humanized groove, and eleven wave types that cover everything from master clock to FM drone, all contained in 8HP. The module never stopped being a clock source. It expanded until the clock source and the modulation infrastructure were the same thing.
